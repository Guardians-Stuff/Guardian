...
module.exports = {
    data: new Discord.SlashCommandSubcommandBuilder()
        ...
        .addUserOption(option => option
            ...
            .setName('channel')
            .setDescription('Ticket to remove the user from.')
        ),
    /**
    ...
    async execute(interaction, client) {
        ...
        if(ticket.user == user.id) return EmbedGenerator.errorEmbed('Unable to remove the ticket creator.')

        if(!member) return EmbedGenerator.errorEmbed('Member not found.');

        channel.permissionOverwrites.edit(member.id, { ViewChannel: false, SendMessages: false }).then(() => {

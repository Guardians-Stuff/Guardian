...
module.exports = {
    data: new Discord.SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName('type').setDescription('The type of audit log to display').setChoices(
                ...
                {
                    name: 'message-delete',
                    value: Discord.AuditLogEvent.MessageDelete.toString(),
                },
                {
                    name: 'message-delete-bulk',
                    value: Discord.AuditLogEvent.MessageBulkDelete.toString(),
                },
                ...
                {
                    name: 'channel-create',
                    value: Discord.AuditLogEvent.ChannelCreate.toString(),
                },
                {
                    name: 'channel-delete',
                    value: Discord.AuditLogEvent.ChannelDelete.toString(),
                },
                {
                    name: 'channel-update',
                    value: Discord.AuditLogEvent.ChannelUpdate.toString(),
                },
                ...
                {
                    name: 'webhook-create',
                    value: Discord.AuditLogEvent.WebhookCreate.toString(),
                },
                {
                    name: 'webhook-delete',
                    value: Discord.AuditLogEvent.WebhookDelete.toString(),
                },
                {
                    name: 'webhook-update',
                    value: Discord.AuditLogEvent.WebhookUpdate.toString(),
                },
    ...
    async execute(interaction) {
        ...
        for (const entry of auditLogs.entries.values()) {
            description.push(
                `**${Discord.AuditLogEvent[entry.action]}** | <@${entry.executor.id}> ${
                    entry.target.username ? ` => <@${entry.target.id}>` : ''
                }`
            );
        }

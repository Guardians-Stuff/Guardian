...
module.exports = {
    data: new Discord.SlashCommandBuilder()
        ...
        .addStringOption(option => option
            .setName('title')
            .setDescription('Title to use for the embed.')
            .setRequired(true)
        ).addRoleOption(option => option.setName('role1').setDescription('Reaction Role.').setRequired(true))
    ...
    async execute(interaction, client, dbGuild) {
        ...
        /** @type {Discord.TextChannel} */ (interaction.channel).send({ embeds: [
            EmbedGenerator.basicEmbed(roles.map((role, index) => `**${index + 1}** | ${role}`).join('\n')).setTitle(`${title} | Reaction Roles`)
        ] }).catch(() =>{
            interaction.editReply({ embeds: [ EmbedGenerator.errorEmbed() ] })
        }).then(async (/** @type {Discord.Message} */ sent) => {
            for(let i = 0; i < roles.length; i++) await sent.react([ '1️⃣', '2️⃣', '3️⃣', '4️⃣', '5️⃣', '6️⃣', '7️⃣', '8️⃣', '9️⃣' ][i]); // should probably .catch() this but idk how
            await ReactionRoles.create({ guild: interaction.guild.id, message: sent.id, roles: roles.map(role => role.id) });

            interaction.editReply({ embeds: [ EmbedGenerator.basicEmbed('Reaction Role message created.') ] });
        })
    }
}